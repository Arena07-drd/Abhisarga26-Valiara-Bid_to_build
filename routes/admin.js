...
router.post('/create-user', catchAsync(async (req, res) => {
  ...
  try {
    ...
    await db.run('INSERT INTO users (username, password, role) VALUES (?, ?, ?)', [username, hash, 'team']);
...
router.post('/add-company', upload.single('pdf_doc'), catchAsync(async (req, res) => {
  ...
  try {
    await db.run('INSERT INTO companies (name, description, pdf_url) VALUES (?, ?, ?)', [name, description, pdf_url]);
...
router.post('/assign-bid', catchAsync(async (req, res) => {
  ...
  const existingBid = await db.get('SELECT * FROM bids WHERE company_id = ?', [company_id]);
  ...
  try {
    await db.run('INSERT INTO bids (team_id, company_id, bid_amount) VALUES (?, ?, ?)', [team_id, company_id, amount]);
    await db.run('UPDATE teams SET purse_remaining = purse_remaining - ? WHERE id = ?', [amount, team_id]);
...
router.post('/set-phase', catchAsync(async (req, res) => {
  ...
  const db = await getDb();
  await db.run('UPDATE system_control SET current_phase = ?', [phase]);
...
router.post('/set-live-company', catchAsync(async (req, res) => {
  ...
  if (company_id === 'none') {
    ...
    await db.run('UPDATE system_control SET live_company_id = ?', [company_id]);
...
router.post('/calculate-scores', catchAsync(async (req, res) => {
  ...
  for (let team of teams) {
    const allocations = await db.all('SELECT * FROM allocations WHERE team_id = ?', [team.id]);
    let totalScore = 0;
    
    for (let alloc of allocations) {
      const result = await db.get('SELECT * FROM company_results WHERE company_id = ?', [alloc.company_id]);
      if (result) {
    ...
    await db.run('INSERT INTO scores (team_id, total_score) VALUES (?, ?)', [team.id, totalScore]);
  }
